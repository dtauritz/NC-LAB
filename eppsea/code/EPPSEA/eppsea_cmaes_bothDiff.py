# This file includes the code that uses EPPSEA to evolve a new eveloutionary path update selection scheme for CMA-ES
...
class ModifiedCMAES(purecma.CMAES):
    ...
    def tell_pop(self, arx, fitvals, population, selection_function, use_sorted_genomes):
        ...
        selected_members = selection_function.eppsea_selection_function.select(population, par.mu, 0)
        ...
        self.C.multiply_with(1 - c1a - par.cmu * sum(par.weights[:par.mu]))  # C *= 1 - c1 - cmu * sum(w)
        self.C.addouter(self.pc, par.c1)  # C += c1 * pc * pc^T, so-called rank-one update

        selected_members = selection_function.eppsea_selection_function.select(population, par.mu, 1)
        if use_sorted_genomes:
            selected_members.sort(key=lambda p: p.fitness, reverse=True)
        selected_arx = list(p.genome for p in selected_members)
        ...
        for k, wk in enumerate(par.weights):  # so-called rank-mu update
            if k < par.mu:
                if wk < 0:  # guaranty positive definiteness
                    wk *= N * (self.sigma / self.C.mahalanobis_norm(purecma.minus(selected_arx[k], xold)))**2
                self.C.addouter(purecma.minus(selected_arx[k], xold), wk * par.cmu / self.sigma**2)  # C += wk * cmu * dx * dx^T
